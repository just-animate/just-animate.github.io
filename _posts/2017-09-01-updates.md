...
The ```interpolate``` property can be set in either on a property basis and instructs Just Animate on how to get continous values between each keyframe.  You can even use interpolate to animate viewBox.  The following code has a custom interpolator for ```viewBox``` that parses the left and right keyframes, and then returns a function to return intermediate values at a given offset (0 - 1 representing 0% to 100%). 
